        ...
        console.log('pettyCashQueryResult  '+JSON.stringify(pettyCashQueryResult.rows));
          if(pettyCashQueryResult.rowCount > 0)
          {
              ...
              pettyCashQueryResult.rows.forEach((eachRecord) => {
                ...
                obj.createDdate = strDate;
                obj.editAction = '<button href="#" class="btn btn-primary editPettyCash" id="'+eachRecord.sfid+'" >Edit</button>'
...
router.post('/updatePettCash',(request,response)=>{
  let body = request.body;
    console.log('body  : '+JSON.stringify(body));
    const{billnum,billdate,natureOfexp,amount,hide}=request.body;
    console.log("body " +billnum+' :'+billdate+': '+amount+': '+hide+':'+natureOfexp);
    let updateQuerry = 'UPDATE salesforce.Petty_Cash_Expense__c SET '+
    'bill_no__c = \''+billnum+'\', '+
    'bill_date__c = \''+billdate+'\', '+
    '	nature_of_exp__c = \''+natureOfexp+'\', '+
    'amount__c = \''+amount+'\' '+
...
.then((PetyCashUpdateResult) => {     
         console.log('PetyCashUpdateResult '+JSON.stringify(PetyCashUpdateResult));
         response.send('Success');
})
.catch((updatetError) => {
...
router.get('/getConveyanceVoucherDetail',verify,(request, response) => {
  ...
  let queryText = 'SELECT conVoucher.sfid,conVoucher.activity_code__c,conVoucher.to__c,conVoucher.from__c, conVoucher.Kms_Travelled__c,conVoucher.project_tasks__c ,conVoucher.heroku_image_url__c, conVoucher.amount__c, conVoucher.mode_of_conveyance__c,conVoucher.purpose_of_travel__c, conVoucher.name as conveyancename ,exp.name as expname,conVoucher.createddate '+
...
router.post('/updateConveyance',(request,response)=>{
  let body = request.body;
  console.log('body  : '+JSON.stringify(body));
  const{todate,fromdate,mode,purpose,travelKm,amount,hide}=request.body;
  console.log("body " +todate+' :'+fromdate+': '+amount+': '+':' +purpose+':'+mode+': '+ hide+':'+travelKm);
  let updateQuerry = 'UPDATE salesforce.Conveyance_Voucher__c SET '+
  'to__c = \''+todate+'\', '+
  'from__c = \''+fromdate+'\', '+
  '	purpose_of_travel__c = \''+purpose+'\', '+
  '	mode_of_conveyance__c = \''+mode+'\', '+
  '	kms_travelled__c = \''+travelKm+'\', '+
  'amount__c = \''+amount+'\' '+
 // 'total_amount__c = \''+amount+'\' '+
  'WHERE sfid = $1';
console.log('updateQuerry  '+updateQuerry);
